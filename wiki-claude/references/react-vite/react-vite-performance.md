...
- Dependency Pre-bundling — `optimizeDeps` (2-5x faster cold start)
- Fast Refresh — Estrutura de componentes compatível
- HMR Config — WebSocket, Docker/WSL, polling
...
- VITE_ Prefix — Segurança: só `VITE_` chega ao client
- Mode Files — `.env.development`, `.env.production`, `--mode staging`
- Sensitive Data — Secrets nunca no client

### 6. Bundle Analysis (MEDIUM)

- Bundle Visualizer — `rollup-plugin-visualizer` com treemap interativo
...
- Vite Reference — Config, features, plugins, build/SSR
- Vite Environment API — Multi-environment (Vite 6+)
- Vite Rolldown Migration — Migração Vite 7 → 8
