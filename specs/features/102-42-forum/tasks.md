...
> **Status:** ✅ executada em 2026-07-02 — 28/28 tasks concluídas via coordenação LATTE
> (Lead direto + executors Haiku, janela ≤3). QA: smoke 11/11, store 16 testes live PASS,
> handler 11 bordas PASS, 22 cenários BDD. Métricas em `metrics.md`.

---

## Fase 0: Débito Técnico (Feature 101)

- [x] **T028:** Corrigir DT-101.2 — remover avatar e login duplicados do `UserSignature.tsx`; manter apenas tier badge + contagem de mensagens
  - **Papel:** executor
  - **agent:** executor3
  - **depends_on:** []
  - **Paralelizável:** true
...
- [x] **T001:** Criar migration `002_add_forum.sql` — ALTER users (title, skills), tabelas boards/board_staff/threads/posts, índices GIN, seed 5 boards
  - **Papel:** executor
  - **agent:** executor1
  - **depends_on:** []
  - **Paralelizável:** true
...
- [x] **T002:** Criar Go models em `internal/forum/model/` — Board, Thread, Post, BoardStaff com UUIDv7
  - **Papel:** executor
  - **agent:** executor2
  - **depends_on:** []
  - **Paralelizável:** true
...
- [x] **T003:** Configurar Tailwind com Design System 42 (cores oficiais, Futura PT, border-radius:0)
  - **Papel:** executor
  - **agent:** executor3
  - **depends_on:** []
  - **Paralelizável:** true
...
- [x] **T004:** Criar `internal/forum/store/boards.go` — CRUD boards, validação slug, blacklist slugs reservados, SeedBoards
  - **Papel:** executor
  - **agent:** executor1
...
- [x] **T005:** Criar `internal/forum/store/board_staff.go` — Add/Remove/GetRole
  - **Papel:** executor
  - **agent:** executor2
  - **depends_on:** [T001, T002]
  - **Paralelizável:** true
...
- [x] **T006:** Criar `internal/forum/store/threads.go` — CRUD threads, bump order (`last_post_at`), GIN tags
  - **Papel:** executor
  - **agent:** executor1
  - **depends_on:** [T001, T002]
  - **Paralelizável:** true
...
- [x] **T007:** Criar `internal/forum/store/posts.go` — CRUD posts, reply-to tree, soft delete
  - **Papel:** executor
  - **agent:** executor2
  - **depends_on:** [T001, T002]
  - **Paralelizável:** true
...
- [x] **T008:** Criar `internal/forum/handler/boards.go` — POST/GET/PATCH/DELETE /api/forum/boards
  - **Papel:** executor
  - **agent:** executor1
...
- [x] **T009:** Criar `internal/forum/handler/threads.go` — POST/GET/PATCH/DELETE /api/forum/threads + /boards/{slug}/threads
  - **Papel:** executor
  - **agent:** executor2
  - **depends_on:** [T006]
  - **Paralelizável:** true
...
- [x] **T010:** Criar `internal/forum/handler/posts.go` — POST /api/forum/threads/{id}/posts, DELETE /api/forum/posts/{id}
  - **Papel:** executor
  - **agent:** executor1
  - **depends_on:** [T007]
  - **Paralelizável:** false
...
- [x] **T011:** Criar `internal/forum/middleware/auth.go` — AuthRequired, ModOnly, AdminOnly, BoardOwner; getBoardID resolve UUID de thread também
  - **Papel:** executor
  - **agent:** executor2
  - **depends_on:** [T004, T005]
  - **Paralelizável:** true
...
- [x] **T012:** Criar `internal/forum/routes/routes.go` + montar subrouter em `cmd/server/main.go` com JWT middleware em todas as rotas autenticadas
  - **Papel:** executor
  - **agent:** executor1
...
- [x] **T013:** Criar `frontend/src/stores/forumStore.ts` — boards, threads, posts, fetchBoards, fetchThreads, createThread, clearError
  - **Papel:** executor
  - **agent:** executor1
  - **depends_on:** [T012]
  - **Paralelizável:** true
...
- [x] **T014:** Criar `frontend/src/components/forum/MDXRenderer.tsx` e `MDXEditor.tsx` — renderização + editor com preview + toolbar
  - **Papel:** executor
  - **agent:** executor2
  - **depends_on:** [T003]
  - **Paralelizável:** true
...
- [x] **T015:** Criar `frontend/src/components/forum/PostCard.tsx` — avatar, login, badge de título, conteúdo MDX, reply-to
  - **Papel:** executor
  - **agent:** executor1
  - **depends_on:** [T014]
  - **Paralelizável:** true
...
- [x] **T016:** Criar `frontend/src/pages/forum/ForumList.tsx` e `BoardCard.tsx` — grid de boards, navegação para /forum/{slug}
  - **Papel:** executor
  - **agent:** executor2
  - **depends_on:** [T013]
  - **Paralelizável:** true
...
- [x] **T017:** Criar `frontend/src/pages/forum/BoardView.tsx` e `ThreadRow.tsx` — lista de threads em bump order
  - **Papel:** executor
  - **agent:** executor1
  - **depends_on:** [T013]
  - **Paralelizável:** true
...
- [x] **T018:** Criar `frontend/src/pages/forum/ThreadView.tsx` — OP + posts em árvore, error handling sem loop de renderização
  - **Papel:** executor
  - **agent:** executor2
...
- [x] **T019:** Criar `frontend/src/pages/forum/NewThread.tsx` e `TagInput.tsx` — formulário de criação com autocomplete de skills
  - **Papel:** executor
  - **agent:** executor1
...
- [x] **T020:** Atualizar `internal/auth/handler.go` — buscar `/v2/users/:id/titles` e `/v2/users/:id/tags_users` no OAuth2 callback; salvar em `users.title` e `users.skills`
  - **Papel:** executor
  - **agent:** executor1
  - **depends_on:** [T001]
  - **Paralelizável:** false
...
- [x] **T021:** Criar `frontend/src/components/forum/ModControls.tsx` — Pin/Lock/Delete condicional por role (mod/owner/admin)
  - **Papel:** executor
  - **agent:** executor2
  - **depends_on:** [T013]
  - **Paralelizável:** true
...
- [x] **T022:** Rodar smoke test `tests/forum_smoke_test.sh` — 11 cenários via curl. Corrigir bugs encontrados: SeedBoards sem board_staff, getBoardID sem thread UUID, JWT middleware ausente nas rotas
  - **Papel:** executor
  - **agent:** executor1
  - **depends_on:** [T012]
  - **Paralelizável:** false
...
- [x] **T023:** Testes unitários store: `go test ./internal/forum/...` — 24 testes (compilam; skip por pg_hba Docker)
  - **Papel:** executor
  - **agent:** executor2
...
- [x] **T024:** Testes de borda handler: `go test ./internal/forum/handler/...` — slug reservado, thread locked, content ≤10k (11/11 PASS)
  - **Papel:** executor
  - **agent:** executor1
...
- [x] **T025:** Escrever `specs/features/102-42-forum/acceptance/forum.feature` — 22 cenários BDD (Gherkin PT-BR)
  - **Papel:** executor
  - **agent:** executor2
  - **depends_on:** [T022]
  - **Paralelizável:** true
...
- [x] **T026:** Atualizar vault wiki: criar `wiki-claude/projects/42_chat/features/feature-102-forum.md` com ADRs, schema, métricas LATTE
  - **Papel:** executor
  - **agent:** executor1
...
- [x] **T027:** Registrar métricas de execução LATTE em `specs/features/102-42-forum/metrics.md`
  - **Papel:** Lead
  - **agent:** Lead
  - **depends_on:** [T026]
  - **Paralelizável:** false
