---
name: wiki-tags
description: Enforce consistent tagging across the Obsidian wiki using a controlled vocabulary at _meta/taxonomy.md. Modes — audit (report state), normalize (fix aliases), new-page (choose tags when creating a page), new-tag (add to vocabulary).
when_to_use: Use when the user says "fix my tags", "normalize tags", "clean up tags", "tag audit", "what tags should I use", or whenever creating/updating wiki pages and needing to choose tags.
argument-hint: "[audit | normalize | new-tag]"
...
- Never add `visibility/internal` just because content is technical
...
**`hot.md`** — Update **Recent Activity** with a one-line summary — e.g. "Tag audit: normalized 14 tags across 28 pages; 2 new canonical tags added." Keep the last 3 operations. Update `updated` timestamp.
