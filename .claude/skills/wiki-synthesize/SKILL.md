...
description: Discover synthesis opportunities across the wiki — pairs or clusters of concepts that co-occur frequently across pages but have no synthesis/ page connecting them — and create cross-cutting synthesis pages with explicit insights.
when_to_use: Use when the user says "synthesize my wiki", "find connections", "what concepts keep coming up together", or after a large ingest when the vault has grown significantly.
...
When writing internal links in synthesis pages, apply the link format from `${CLAUDE_SKILL_DIR}/../wiki-meta/SKILL.md` (Link Format section) using the `OBSIDIAN_LINK_FORMAT` value.
...
Aim for the top 20-30 pairs by co-occurrence score. Use Grep to find backlinks efficiently:
...
Pick the top 5 candidates. If the user asked for a specific topic, filter candidates to that domain first.
...
After creating pages for the top 5, list the next 10 candidates in your output:
...
**`hot.md`** — Update **Recent Activity** with what was synthesized. Update **Active Threads** with any open questions the synthesis surfaced. Update `updated` timestamp.
...
- **Three-way syntheses are powerful but rare.** Only create them when three concepts form a genuine triangle of mutual influence.
- **Check `_insights.md` first.** The `wiki-status` skill may have already flagged synthesis candidates there — start with those.
