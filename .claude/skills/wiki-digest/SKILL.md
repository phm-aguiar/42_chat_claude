---
name: wiki-digest
description: Generate a periodic knowledge digest — a newsletter-style summary of what was learned, updated, and connected in the wiki over a time period (day/week/month). Distinct from wiki-status (which reports source delta) — this summarizes knowledge, not ingestion state.
when_to_use: Use when the user says "what did I learn this week", "give me a digest", "weekly summary", "knowledge report", "what's new in my wiki", or "/wiki-digest [period]".
...
3. Read `$OBSIDIAN_VAULT_PATH/log.md` — last 200 lines — for entries within the period.
4. Read `$OBSIDIAN_VAULT_PATH/hot.md` for current session context.
5. If `$OBSIDIAN_VAULT_PATH/_insights.md` exists, read its **Anchor Pages** table.
...
Scan new and updated pages for cross-category wikilinks — links that bridge different knowledge layers.

For each active page, extract all `[[wikilink]]` targets. Classify each link by the target's category prefix. Flag links that cross categories.
...
- **Ambiguous claims**: count `^[ambiguous]` markers across all active pages (just the count and which pages have the most)
- **Unstaged notes**: count files in `$OBSIDIAN_VAULT_PATH/_raw/`
- **Taxonomy gaps**: tags from Step 2 that aren't in `_meta/taxonomy.md`

## Step 5: Choose Recommended Re-reads

From the *existing* (pre-period) pages, identify 2–3 worth revisiting given this period's new context.

Heuristic: find pre-period pages that share the most tags with the active pages from Step 1. Also include any pre-period page that now has 2+ new incoming links from active pages.
...
Apply the link format from `${CLAUDE_SKILL_DIR}/../wiki-meta/SKILL.md` (Link Format section) using `OBSIDIAN_LINK_FORMAT`. Default is `[[wikilink]]`.
...
- **Unstaged notes**: [N] files in `_raw/` — run `/wiki-ingest` to promote them
- **Taxonomy gaps**: Tags `#newtag1`, `#newtag2` used but not in taxonomy — run `/wiki-tags`
...
**Visibility**: If a page is tagged `visibility/pii`, exclude it from all tables and connection lists (but count it in totals, noted as "+ N private"). If the user explicitly says "include private pages", include them normally.
...
**If user prefixed with "save" or "write":**
- Write to `$OBSIDIAN_VAULT_PATH/journal/digest-YYYY-MM-DD.md`
- Add frontmatter (title, category: journal, tags: [digest, meta/review], sources: [], created, updated, summary)
...
- **Headlines are the payoff.** Don't list page titles — synthesize the actual learning.
...
- **This skill only reads.** The only writes are the optional journal page and the `log.md` append.
- **Don't duplicate wiki-status.** If the user asks "what needs ingesting", route to `wiki-status`. This skill answers "what did I learn".
